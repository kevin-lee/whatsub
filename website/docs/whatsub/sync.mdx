...
```shell
whatsub sync --sub-type smi --sync +5s --src sub.smi --out sub-synced.smi
```
Or
```shell
whatsub sync -t smi -m +5s -s sub.smi -o sub-synced.smi
```
...
```shell
whatsub sync --sub-type srt --sync +5s --src sub.srt --out sub-synced.srt
```
Or
```shell
whatsub sync -t srt -m +5s -s sub.srt -o sub-synced.srt
```
...
```shell
Usage:
  Whatsub sync -t|--sub-type <sub-type> -m|--sync <sync> -s|--src <src> [-o|--out <out>] [-h|--help HELP]

sync subtitles

Available options:
  -t|--sub-type <sub-type> A type of subtitle. Either smi or srt
